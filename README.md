...
이를 위해 LiDAR 포인트클라우드를 갭(gap) 기준으로 클러스터링한 후, Split-and-Merge와 PCA를 이용해 직선(벽면)으로 피팅하고, 마주보는 벽 쌍(facing-pair)을 검증하여 좌/우 벽면으로 라벨링한다. 이렇게 인식된 좌·우 벽면을 기준으로 Stanley 조향 제어기(횡방향 오차 부호 보정 적용)를 통해 로봇의 위치와 방향을 보정한다.

정렬 완료(정지) 여부는 투영 거리가 아닌 전방향 최소 유클리드 거리('min_omnidirectional_distance()')를 기준으로 판단한다. 투영 거리는 실제 거리보다 작게 계산될 수 있어 정지 기준으로 부적합하기 때문이다.

진입(ENTER) 절차는 "DRIVING → BACKOFF → ALIGNING → (retry) → SETTLING → DONE" 상태로 구성된 FSM으로 관리되며, 제자리 회전 시 로봇 몸체가 주변 구조물과 충돌하는 것을 방지하기 위해 회전 전 후진(Back-off) 동작을 거치도록 설계하였다.
...
Camera Image  +  Joint State / Action
...
수집한 데이터를 LeRobot Dataset으로 구성하고 ACT Policy로 학습하여 Follower Robot Arm이 사람의 청소 동작을 재현하도록 구현하였다. 
이러한 Leader/Follower 기반 시연 수집과 ACT 적용 방식은 프로젝트 초기 모방학습 설계의 핵심 구조이다.
